...
class SvgImageCache {
  ...
  get(url: string): HTMLImageElement | null {
    return this.cache.get(this.cacheKey(url)) ?? null;
  }
  ...
  hasFailed(url: string): boolean {
    return this.failed.has(this.cacheKey(url));
  }
  ...
  private cacheKey(url: string): string {
    return import.meta.env.DEV ? `${url}::${this.revision}` : url;
  }
...
if (import.meta.hot) {
  const onSvgAssetsChanged = () => {
    sharedImageCache.clear();
  };
  import.meta.hot.on("ascension:svg-assets-changed", onSvgAssetsChanged);
  import.meta.hot.dispose(() => {
    import.meta.hot?.off("ascension:svg-assets-changed", onSvgAssetsChanged);
  });
}
