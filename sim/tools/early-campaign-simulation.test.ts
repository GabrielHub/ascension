...
describe("early campaign simulation harness", () => {
  it(
    "is deterministic for the same seed window",
    async () => {
  ...
  it(
    "captures required metrics and threshold summaries",
    async () => {
  ...
  it(
    "does not invert lower-bound threshold checks for positive measurements",
    async () => {
      const suite = await buildEarlyCampaignSimulationSuite({ seedCount: 1, contractLimit: 1 });
      const firstRun = suite.runs[0];
  ...
  it(
    "records boss contact whenever a contract records a boss clear",
    async () => {
      const suite = await buildEarlyCampaignSimulationSuite({ seedCount: 1, contractLimit: 1 });
      const firstRun = suite.runs[0];
  ...
  it(
    "keeps seeded opener runs moving and surfaces recruit timing in the opening window",
    async () => {
  ...
  it(
    "renders self-consistent simulation artifacts",
    async () => {
