import type { DropTable, ItemTemplate, PrepRecipeTemplate } from "./shared";

export const itemTemplates = [
  // --- Weapons ---
  {
    id: "weapon/pipe-wrench",
    kind: "item",
    name: "Pipe Wrench",
    tags: ["rank:f"],
    description: "A heavy wrench repurposed for close combat.",
    category: "weapon",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 30,
    sellPrice: 12,
    statEffects: [{ stat: "strength", value: 3 }],
  },
  {
    id: "weapon/kitchen-knife",
    kind: "item",
    name: "Kitchen Knife",
    tags: ["rank:f"],
    description: "A sharp blade from the bodega kitchen drawer.",
    category: "weapon",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 25,
    sellPrice: 10,
    statEffects: [
      { stat: "speed", value: 2 },
      { stat: "strength", value: 1 },
    ],
  },
  {
    id: "weapon/baseball-bat",
    kind: "item",
    name: "Baseball Bat",
    tags: ["rank:f"],
    description: "Reliable blunt force in a familiar shape.",
    category: "weapon",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 35,
    sellPrice: 14,
    statEffects: [{ stat: "strength", value: 4 }],
  },
  {
    id: "weapon/tactical-knife",
    kind: "item",
    name: "Tactical Knife",
    tags: ["rank:e"],
    description: "A purpose-built combat knife with a reinforced grip.",
    category: "weapon",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 60,
    sellPrice: 25,
    statEffects: [
      { stat: "speed", value: 3 },
      { stat: "strength", value: 2 },
    ],
  },
  {
    id: "weapon/machete",
    kind: "item",
    name: "Machete",
    tags: ["rank:e"],
    description: "A long, heavy blade suited for clearing paths and threats.",
    category: "weapon",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 75,
    sellPrice: 30,
    statEffects: [{ stat: "strength", value: 5 }],
  },
  {
    id: "weapon/compact-crossbow",
    kind: "item",
    name: "Compact Crossbow",
    tags: ["rank:e"],
    description: "A small crossbow offering ranged precision at short distances.",
    category: "weapon",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 85,
    sellPrice: 35,
    statEffects: [
      { stat: "speed", value: 2 },
      { stat: "perception", value: 2 },
      { stat: "strength", value: 2 },
    ],
  },
  {
    id: "weapon/reinforced-bat",
    kind: "item",
    name: "Reinforced Bat",
    tags: ["rank:d"],
    description: "A steel-core bat wrapped in shock-absorbing tape.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 120,
    sellPrice: 50,
    statEffects: [
      { stat: "strength", value: 6 },
      { stat: "resilience", value: 2 },
    ],
  },
  {
    id: "weapon/katana",
    kind: "item",
    name: "Katana",
    tags: ["rank:d"],
    description: "A balanced blade that rewards skilled hands.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 150,
    sellPrice: 60,
    statEffects: [
      { stat: "speed", value: 4 },
      { stat: "strength", value: 5 },
    ],
  },
  {
    id: "weapon/tactical-rifle",
    kind: "item",
    name: "Tactical Rifle",
    tags: ["rank:d"],
    description: "A modular rifle platform built for field operations.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 180,
    sellPrice: 75,
    statEffects: [
      { stat: "strength", value: 4 },
      { stat: "speed", value: 3 },
      { stat: "perception", value: 3 },
    ],
  },
  {
    id: "weapon/dual-daggers",
    kind: "item",
    name: "Dual Daggers",
    tags: ["rank:d"],
    description: "A matched pair of lightweight combat daggers.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 140,
    sellPrice: 55,
    statEffects: [
      { stat: "speed", value: 6 },
      { stat: "strength", value: 3 },
    ],
  },

  {
    id: "weapon/crowbar",
    kind: "item",
    name: "Crowbar",
    tags: ["rank:f"],
    description: "A pry bar heavy enough to discourage anything with joints.",
    category: "weapon",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 30,
    sellPrice: 12,
    statEffects: [
      { stat: "strength", value: 2 },
      { stat: "resilience", value: 1 },
    ],
  },
  {
    id: "weapon/box-cutter",
    kind: "item",
    name: "Box Cutter",
    tags: ["rank:f"],
    description: "A retractable utility blade. Fast, disposable, unpleasant.",
    category: "weapon",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 20,
    sellPrice: 8,
    statEffects: [{ stat: "speed", value: 3 }],
  },
  // ── F-rank weapons (continued) ──────────────────────────────────────────
  {
    id: "weapon/tire-iron",
    kind: "item",
    name: "Tire Iron",
    tags: ["rank:f"],
    description:
      "Bent from a trunk kit. The curve makes it worse for tires and better for everything else.",
    category: "weapon",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 28,
    sellPrice: 11,
    statEffects: [
      { stat: "strength", value: 3 },
      { stat: "endurance", value: 1 },
    ],
  },
  {
    id: "weapon/claw-hammer",
    kind: "item",
    name: "Claw Hammer",
    tags: ["rank:f"],
    description: "Standard-issue framing hammer. The claw end is not decorative.",
    category: "weapon",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 22,
    sellPrice: 9,
    statEffects: [{ stat: "strength", value: 2 }],
  },
  {
    id: "weapon/chain-length",
    kind: "item",
    name: "Chain Length",
    tags: ["rank:f"],
    description: "Three feet of hardware chain with a padlock on one end. Flexible reach.",
    category: "weapon",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 18,
    sellPrice: 7,
    statEffects: [
      { stat: "speed", value: 2 },
      { stat: "strength", value: 1 },
    ],
  },
  // ── E-rank weapons ──────────────────────────────────────────────────────
  {
    id: "weapon/nail-bat",
    kind: "item",
    name: "Nail Bat",
    tags: ["rank:e"],
    description:
      "A baseball bat with roofing nails driven through the barrel. Field-modified and ugly.",
    category: "weapon",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 55,
    sellPrice: 22,
    statEffects: [
      { stat: "strength", value: 5 },
      { stat: "speed", value: -1 },
    ],
  },
  {
    id: "weapon/stun-baton",
    kind: "item",
    name: "Stun Baton",
    tags: ["rank:e"],
    description: "A collapsible baton with a jury-rigged electric tip. Brief but persuasive.",
    category: "weapon",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 80,
    sellPrice: 32,
    statEffects: [
      { stat: "speed", value: 3 },
      { stat: "strength", value: 3 },
    ],
  },
  {
    id: "weapon/rebar-spike",
    kind: "item",
    name: "Rebar Spike",
    tags: ["rank:e"],
    description: "A sharpened length of rebar pulled from a condemned site. Heavier than it looks.",
    category: "weapon",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 65,
    sellPrice: 26,
    statEffects: [
      { stat: "strength", value: 4 },
      { stat: "resilience", value: 1 },
    ],
  },
  {
    id: "weapon/hand-axe",
    kind: "item",
    name: "Hand Axe",
    tags: ["rank:e"],
    description: "A compact hatchet built for clearing obstacles and threats alike.",
    category: "weapon",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 70,
    sellPrice: 28,
    statEffects: [
      { stat: "strength", value: 4 },
      { stat: "speed", value: 1 },
    ],
  },
  {
    id: "weapon/sidearm",
    kind: "item",
    name: "Sidearm",
    tags: ["rank:d"],
    description: "A reliable semi-automatic pistol for operators who prefer range over reach.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 160,
    sellPrice: 65,
    statEffects: [
      { stat: "speed", value: 4 },
      { stat: "perception", value: 3 },
      { stat: "strength", value: 2 },
    ],
  },
  // ── D-rank weapons (rift-touched) ───────────────────────────────────
  {
    id: "weapon/rift-scored-blade",
    kind: "item",
    name: "Rift-Scored Blade",
    tags: ["rank:d"],
    description:
      "A machete whose edge developed micro-fissures from prolonged rift exposure. Cuts follow trajectories that shouldn't be possible.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 170,
    sellPrice: 68,
    statEffects: [
      { stat: "strength", value: 6 },
      { stat: "speed", value: 3 },
    ],
  },
  {
    id: "weapon/pulse-baton",
    kind: "item",
    name: "Pulse Baton",
    tags: ["rank:d"],
    description:
      "A riot baton with a salvaged rift-crystal emitter seated in the grip. The impact reverberates at a frequency that disrupts coordination.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 58,
    statEffects: [
      { stat: "strength", value: 4 },
      { stat: "speed", value: 3 },
      { stat: "perception", value: 2 },
    ],
  },
  {
    id: "weapon/breach-hammer",
    kind: "item",
    name: "Breach Hammer",
    tags: ["rank:d"],
    description:
      "A two-handed sledge built for structural entry in rift sites. The head hums at a frequency that weakens material bonds on contact.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 76,
    statEffects: [
      { stat: "strength", value: 8 },
      { stat: "resilience", value: 2 },
      { stat: "speed", value: -1 },
    ],
  },
  {
    id: "weapon/focused-carbine",
    kind: "item",
    name: "Focused Carbine",
    tags: ["rank:d"],
    description:
      "A short-barreled rifle with a rift-glass reflex sight. In rift-dense air, the rounds track slightly toward center mass.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 80,
    statEffects: [
      { stat: "strength", value: 4 },
      { stat: "perception", value: 4 },
      { stat: "speed", value: 2 },
    ],
  },

  // --- Outfit Overlays ---
  {
    id: "outfit-overlay/padded-jacket",
    kind: "item",
    name: "Padded Jacket",
    tags: ["rank:f"],
    description: "A thick jacket offering basic protection against impacts.",
    category: "outfit-overlay",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 40,
    sellPrice: 16,
    statEffects: [
      { stat: "resilience", value: 2 },
      { stat: "endurance", value: 1 },
    ],
  },
  {
    id: "outfit-overlay/leather-vest",
    kind: "item",
    name: "Leather Vest",
    tags: ["rank:f"],
    description: "A rugged leather vest that absorbs light blows.",
    category: "outfit-overlay",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 35,
    sellPrice: 14,
    statEffects: [{ stat: "resilience", value: 3 }],
  },
  {
    id: "outfit-overlay/tactical-vest",
    kind: "item",
    name: "Tactical Vest",
    tags: ["rank:e"],
    description: "A purpose-built vest with plating and utility pockets.",
    category: "outfit-overlay",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 80,
    sellPrice: 32,
    statEffects: [
      { stat: "resilience", value: 4 },
      { stat: "endurance", value: 2 },
    ],
  },
  {
    id: "outfit-overlay/stealth-cloak",
    kind: "item",
    name: "Stealth Cloak",
    tags: ["rank:e"],
    description: "A lightweight cloak that dampens sound and movement.",
    category: "outfit-overlay",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 90,
    sellPrice: 36,
    statEffects: [
      { stat: "speed", value: 3 },
      { stat: "resilience", value: 2 },
    ],
  },
  {
    id: "outfit-overlay/heavy-plate",
    kind: "item",
    name: "Heavy Plate",
    tags: ["rank:d"],
    description: "A layered armor plate that trades mobility for serious protection.",
    category: "outfit-overlay",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 160,
    sellPrice: 65,
    statEffects: [
      { stat: "resilience", value: 7 },
      { stat: "endurance", value: 3 },
    ],
  },
  {
    id: "outfit-overlay/field-medic-coat",
    kind: "item",
    name: "Field Medic Coat",
    tags: ["rank:d"],
    description: "A reinforced medical coat with integrated supply pouches.",
    category: "outfit-overlay",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 55,
    statEffects: [
      { stat: "resilience", value: 4 },
      { stat: "endurance", value: 4 },
      { stat: "perception", value: 2 },
    ],
  },

  {
    id: "outfit-overlay/work-coveralls",
    kind: "item",
    name: "Work Coveralls",
    tags: ["rank:f"],
    description:
      "Tough full-body coveralls meant for industrial sites. Not fashionable. Functional.",
    category: "outfit-overlay",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 30,
    sellPrice: 12,
    statEffects: [
      { stat: "endurance", value: 2 },
      { stat: "resilience", value: 1 },
    ],
  },
  // ── F-rank outfits (continued) ─────────────────────────────────────────
  {
    id: "outfit-overlay/denim-jacket",
    kind: "item",
    name: "Denim Jacket",
    tags: ["rank:f"],
    description:
      "A thick denim jacket with the sleeves intact. Turns a glancing blow into a bruise.",
    category: "outfit-overlay",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 32,
    sellPrice: 13,
    statEffects: [
      { stat: "resilience", value: 2 },
      { stat: "speed", value: 1 },
    ],
  },
  {
    id: "outfit-overlay/hi-vis-vest",
    kind: "item",
    name: "Hi-Vis Vest",
    tags: ["rank:f"],
    description: "Reflective safety vest. Offers negligible protection but makes you easy to find.",
    category: "outfit-overlay",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 15,
    sellPrice: 6,
    statEffects: [
      { stat: "perception", value: 2 },
      { stat: "endurance", value: 1 },
    ],
  },
  // ── E-rank outfits ─────────────────────────────────────────────────────
  {
    id: "outfit-overlay/insulated-parka",
    kind: "item",
    name: "Insulated Parka",
    tags: ["rank:e"],
    description:
      "A heavy cold-weather parka with synthetic fill. Slows you down but shrugs off impacts.",
    category: "outfit-overlay",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 70,
    sellPrice: 28,
    statEffects: [
      { stat: "resilience", value: 4 },
      { stat: "endurance", value: 1 },
      { stat: "speed", value: -1 },
    ],
  },
  {
    id: "outfit-overlay/courier-jacket",
    kind: "item",
    name: "Courier Jacket",
    tags: ["rank:e"],
    description:
      "A bike messenger jacket with built-in armor panels. Designed for people who get hit by cars.",
    category: "outfit-overlay",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 78,
    sellPrice: 31,
    statEffects: [
      { stat: "speed", value: 3 },
      { stat: "resilience", value: 2 },
    ],
  },
  {
    id: "outfit-overlay/reinforced-hoodie",
    kind: "item",
    name: "Reinforced Hoodie",
    tags: ["rank:e"],
    description:
      "A lined hoodie with concealed plating. Moves like street clothes, stops like armor.",
    category: "outfit-overlay",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 75,
    sellPrice: 30,
    statEffects: [
      { stat: "resilience", value: 3 },
      { stat: "speed", value: 2 },
    ],
  },
  {
    id: "outfit-overlay/ballistic-vest",
    kind: "item",
    name: "Ballistic Vest",
    tags: ["rank:d"],
    description: "A rated protective vest with ceramic inserts. Heavy but confidence-inspiring.",
    category: "outfit-overlay",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 150,
    sellPrice: 60,
    statEffects: [
      { stat: "resilience", value: 6 },
      { stat: "endurance", value: 2 },
      { stat: "speed", value: 1 },
    ],
  },
  // ── D-rank outfits (rift-touched) ──────────────────────────────────
  {
    id: "outfit-overlay/rift-lined-jacket",
    kind: "item",
    name: "Rift-Lined Jacket",
    tags: ["rank:d"],
    description:
      "A tactical jacket with rift-fiber woven into the lining. Impact energy dissipates in directions that don't exist.",
    category: "outfit-overlay",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 175,
    sellPrice: 70,
    statEffects: [
      { stat: "resilience", value: 6 },
      { stat: "speed", value: 2 },
      { stat: "endurance", value: 2 },
    ],
  },
  {
    id: "outfit-overlay/signal-dampening-coat",
    kind: "item",
    name: "Signal-Dampening Coat",
    tags: ["rank:d"],
    description:
      "A long coat treated with compounds that block rift-frequency interference. Feels colder than ambient by exactly two degrees.",
    category: "outfit-overlay",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 62,
    statEffects: [
      { stat: "resilience", value: 5 },
      { stat: "perception", value: 3 },
      { stat: "endurance", value: 1 },
    ],
  },
  {
    id: "outfit-overlay/composite-plate-carrier",
    kind: "item",
    name: "Composite Plate Carrier",
    tags: ["rank:d"],
    description:
      "Military surplus with rift-ceramic inserts. The inserts glow faintly at night and nobody can explain why.",
    category: "outfit-overlay",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 74,
    statEffects: [
      { stat: "resilience", value: 8 },
      { stat: "endurance", value: 2 },
      { stat: "speed", value: -1 },
    ],
  },

  // --- Accessories ---
  {
    id: "accessory/comm-earpiece",
    kind: "item",
    name: "Comm Earpiece",
    tags: ["rank:f"],
    description: "A discreet earpiece for field communication.",
    category: "accessory",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 20,
    sellPrice: 8,
    statEffects: [{ stat: "perception", value: 2 }],
  },
  {
    id: "accessory/tactical-scarf",
    kind: "item",
    name: "Tactical Scarf",
    tags: ["rank:f"],
    description: "A multi-purpose scarf that shields against dust and debris.",
    category: "accessory",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 15,
    sellPrice: 6,
    statEffects: [
      { stat: "resilience", value: 1 },
      { stat: "speed", value: 1 },
    ],
  },
  {
    id: "accessory/eye-visor",
    kind: "item",
    name: "Eye Visor",
    tags: ["rank:e"],
    description: "An optical visor that enhances threat detection.",
    category: "accessory",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 55,
    sellPrice: 22,
    statEffects: [{ stat: "perception", value: 4 }],
  },
  {
    id: "accessory/medkit-pouch",
    kind: "item",
    name: "Medkit Pouch",
    tags: ["rank:e"],
    description: "A compact medical pouch worn on the belt.",
    category: "accessory",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 65,
    sellPrice: 26,
    statEffects: [
      { stat: "resilience", value: 2 },
      { stat: "endurance", value: 2 },
    ],
  },
  {
    id: "accessory/scout-binocs",
    kind: "item",
    name: "Scout Binocs",
    tags: ["rank:e"],
    description: "Lightweight binoculars for long-range reconnaissance.",
    category: "accessory",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 50,
    sellPrice: 20,
    statEffects: [{ stat: "perception", value: 5 }],
  },
  {
    id: "accessory/field-lead-badge",
    kind: "item",
    name: "Field Lead Badge",
    tags: ["rank:e"],
    description: "An authority badge that bolsters composure under pressure.",
    category: "accessory",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 45,
    sellPrice: 18,
    statEffects: [
      { stat: "resilience", value: 3 },
      { stat: "endurance", value: 1 },
    ],
  },
  {
    id: "accessory/reinforced-gloves",
    kind: "item",
    name: "Reinforced Gloves",
    tags: ["rank:d"],
    description: "Hardened gloves that protect hands and improve grip strength.",
    category: "accessory",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 40,
    statEffects: [
      { stat: "strength", value: 3 },
      { stat: "resilience", value: 3 },
    ],
  },

  {
    id: "accessory/wrist-brace",
    kind: "item",
    name: "Wrist Brace",
    tags: ["rank:f"],
    description: "A rigid wrist support that keeps hands steady under impact.",
    category: "accessory",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 18,
    sellPrice: 7,
    statEffects: [{ stat: "endurance", value: 2 }],
  },
  // ── F-rank accessories (continued) ─────────────────────────────────────
  {
    id: "accessory/knee-pads",
    kind: "item",
    name: "Knee Pads",
    tags: ["rank:f"],
    description: "Construction-grade knee pads. Keeps you moving through crawlspaces and rubble.",
    category: "accessory",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 16,
    sellPrice: 6,
    statEffects: [
      { stat: "endurance", value: 1 },
      { stat: "resilience", value: 1 },
    ],
  },
  {
    id: "accessory/work-gloves",
    kind: "item",
    name: "Work Gloves",
    tags: ["rank:f"],
    description: "Leather-palm work gloves. Not rated for combat but better than bare hands.",
    category: "accessory",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 14,
    sellPrice: 5,
    statEffects: [{ stat: "strength", value: 1 }],
  },
  // ── E-rank accessories ─────────────────────────────────────────────────
  {
    id: "accessory/respirator-mask",
    kind: "item",
    name: "Respirator Mask",
    tags: ["rank:e"],
    description:
      "A half-face respirator that filters spores, fumes, and whatever else dungeons exhale.",
    category: "accessory",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 48,
    sellPrice: 19,
    statEffects: [
      { stat: "endurance", value: 3 },
      { stat: "resilience", value: 1 },
    ],
  },
  {
    id: "accessory/ankle-holster",
    kind: "item",
    name: "Ankle Holster",
    tags: ["rank:e"],
    description:
      "A concealed holster that keeps a backup tool accessible without slowing you down.",
    category: "accessory",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 42,
    sellPrice: 17,
    statEffects: [
      { stat: "speed", value: 2 },
      { stat: "perception", value: 1 },
    ],
  },
  {
    id: "accessory/shoulder-lamp",
    kind: "item",
    name: "Shoulder Lamp",
    tags: ["rank:e"],
    description: "A clip-on tactical light that keeps both hands free in dark sites.",
    category: "accessory",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 50,
    sellPrice: 20,
    statEffects: [
      { stat: "perception", value: 3 },
      { stat: "speed", value: 1 },
    ],
  },
  {
    id: "accessory/utility-belt",
    kind: "item",
    name: "Utility Belt",
    tags: ["rank:d"],
    description: "A modular tool belt with quick-access pouches for field supplies.",
    category: "accessory",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 105,
    sellPrice: 42,
    statEffects: [
      { stat: "endurance", value: 2 },
      { stat: "perception", value: 2 },
      { stat: "speed", value: 2 },
    ],
  },
  {
    id: "accessory/trauma-kit",
    kind: "item",
    name: "Trauma Kit",
    tags: ["rank:d"],
    description:
      "A field-grade trauma response kit worn on the hip. Standard issue for medic-adjacent operators.",
    category: "accessory",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 38,
    statEffects: [
      { stat: "endurance", value: 3 },
      { stat: "resilience", value: 2 },
      { stat: "perception", value: 1 },
    ],
  },
  // ── D-rank accessories (rift-touched) ──────────────────────────────
  {
    id: "accessory/rift-compass",
    kind: "item",
    name: "Rift Compass",
    tags: ["rank:d"],
    description:
      "Points toward rift density instead of north. Spins when you're about to have a bad day.",
    category: "accessory",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 46,
    statEffects: [
      { stat: "perception", value: 4 },
      { stat: "speed", value: 2 },
    ],
  },
  {
    id: "accessory/dampening-earbuds",
    kind: "item",
    name: "Dampening Earbuds",
    tags: ["rank:d"],
    description:
      "Filter out psychic noise from rift entities. Hum faintly when dimensional stress is nearby.",
    category: "accessory",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 110,
    sellPrice: 44,
    statEffects: [
      { stat: "endurance", value: 3 },
      { stat: "resilience", value: 2 },
      { stat: "perception", value: 1 },
    ],
  },
  {
    id: "accessory/stabilizer-gauntlet",
    kind: "item",
    name: "Stabilizer Gauntlet",
    tags: ["rank:d"],
    description:
      "Wrist-mounted gyroscopic stabilizer. Keeps your hands steady when the space around you isn't.",
    category: "accessory",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 125,
    sellPrice: 50,
    statEffects: [
      { stat: "strength", value: 3 },
      { stat: "speed", value: 2 },
      { stat: "resilience", value: 2 },
    ],
  },

  // --- Monster Part Loot ---
  {
    id: "loot/monster-part/fang",
    kind: "item",
    name: "Fang",
    tags: ["loot:monster_part", "loot:crafting_input"],
    description: "A sharp tooth ripped from a hostile creature.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/carapace",
    kind: "item",
    name: "Carapace",
    tags: ["loot:monster_part", "loot:crafting_input"],
    description: "A thick shell fragment that once shielded a threat.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 12,
    statEffects: [],
  },
  {
    id: "loot/monster-part/ichor",
    kind: "item",
    name: "Ichor",
    tags: ["loot:monster_part", "loot:crafting_input"],
    description: "A viscous fluid harvested from a defeated anomaly.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 6,
    statEffects: [],
  },
  {
    id: "loot/monster-part/bone-shard",
    kind: "item",
    name: "Bone Shard",
    tags: ["loot:monster_part", "loot:crafting_input"],
    description: "A jagged bone fragment from a large creature.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 10,
    statEffects: [],
  },
  {
    id: "loot/monster-part/sinew",
    kind: "item",
    name: "Sinew",
    tags: ["loot:monster_part", "loot:crafting_input"],
    description: "Tough connective tissue with residual tension.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 7,
    statEffects: [],
  },
  {
    id: "loot/monster-part/crystal-eye",
    kind: "item",
    name: "Crystal Eye",
    tags: ["loot:monster_part", "loot:crafting_input", "rank:e"],
    description: "A crystallized eye that refracts ambient light.",
    category: "loot",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 18,
    statEffects: [],
  },
  {
    id: "loot/monster-part/void-residue",
    kind: "item",
    name: "Void Residue",
    tags: ["loot:monster_part", "loot:crafting_input", "rank:d"],
    description: "A dark, shimmering substance from a powerful entity.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 25,
    statEffects: [],
  },
  {
    id: "loot/monster-part/threat-gland",
    kind: "item",
    name: "Threat Gland",
    tags: ["loot:monster_part", "loot:crafting_input", "rank:e"],
    description: "A glandular organ that once powered a creature's aggression.",
    category: "loot",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 15,
    statEffects: [],
  },

  // ── Themed family loot: tunnel-crawlers ───────────────────────────────
  {
    id: "loot/monster-part/drain-sludge",
    kind: "item",
    name: "Drain Sludge",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description: "Viscous residue scraped from tunnel-crawler carapaces. Warm to the touch.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 7,
    statEffects: [],
  },
  {
    id: "loot/monster-part/pipe-scale",
    kind: "item",
    name: "Pipe Scale",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "Mineral-encrusted plating shed inside flooded pipes. Rings when struck.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 11,
    statEffects: [],
  },

  // ── Themed family loot: concrete-sentinels ────────────────────────────
  {
    id: "loot/monster-part/ramp-gravel",
    kind: "item",
    name: "Ramp Gravel",
    tags: ["loot:monster_part", "loot:sell_only", "loot:crafting_input", "loot:dungeon_drop"],
    description: "Animate concrete fragments from parking structure entities. Still vibrates.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/bollard-core",
    kind: "item",
    name: "Bollard Core",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "Dense core material from structural sentinel entities. Heavier than it looks.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 11,
    statEffects: [],
  },

  // ── Themed family loot: chalk-swarms ──────────────────────────────────
  {
    id: "loot/monster-part/chalk-filament",
    kind: "item",
    name: "Chalk Filament",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description: "Fibrous chalk threads from swarm creatures. Crumbles if you grip too hard.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 7,
    statEffects: [],
  },
  {
    id: "loot/monster-part/lesson-fragment",
    kind: "item",
    name: "Lesson Fragment",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "Crystallized educational residue. Words shift when you try to read them.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 12,
    statEffects: [],
  },

  // ── Themed family loot: vine-constructs ───────────────────────────────
  {
    id: "loot/monster-part/root-fiber",
    kind: "item",
    name: "Root Fiber",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description: "Tough fibers from animate root systems. Tries to grow if left in water.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 6,
    statEffects: [],
  },
  {
    id: "loot/monster-part/pollen-sac",
    kind: "item",
    name: "Pollen Sac",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "Intact sac from botanical constructs. Glows faintly in the dark.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 12,
    statEffects: [],
  },

  // ── Themed family loot: suds-constructs ───────────────────────────────
  {
    id: "loot/monster-part/lint-clump",
    kind: "item",
    name: "Lint Clump",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description: "Condensed lint matter from laundered entities. Smells like industrial detergent.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 7,
    statEffects: [],
  },
  {
    id: "loot/monster-part/detergent-crystal",
    kind: "item",
    name: "Detergent Crystal",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "Crystallized chemical compound from suds constructs. Caustic if cracked.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 10,
    statEffects: [],
  },

  // ── Themed family loot: cellar-dwellers ───────────────────────────────
  {
    id: "loot/monster-part/radiator-ash",
    kind: "item",
    name: "Radiator Ash",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description: "Hot ash from heat-dwelling basement creatures. Stays warm for days.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/basement-mold",
    kind: "item",
    name: "Basement Mold",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "Animate fungal matter from sub-level dwellers. Pulses slowly.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 10,
    statEffects: [],
  },

  // ── Themed family loot: court-echoes ──────────────────────────────────
  {
    id: "loot/monster-part/echo-resin",
    kind: "item",
    name: "Echo Resin",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description: "Solidified sound residue from athletic ghosts. Hums when thrown.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 7,
    statEffects: [],
  },
  {
    id: "loot/monster-part/rubber-fragment",
    kind: "item",
    name: "Rubber Fragment",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "Warped rubber shed by court echo entities. Bounces at wrong angles.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 11,
    statEffects: [],
  },

  // ── Themed family loot: freezer-fauna ─────────────────────────────────
  {
    id: "loot/monster-part/frost-shard",
    kind: "item",
    name: "Frost Shard",
    tags: ["loot:monster_part", "loot:sell_only", "loot:dungeon_drop"],
    description: "Razor-sharp ice crystal from cold-adapted fauna. Does not melt.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/preserved-marrow",
    kind: "item",
    name: "Preserved Marrow",
    tags: ["loot:monster_part", "loot:sell_only", "loot:dungeon_drop"],
    description: "Unnaturally preserved organic core from freezer fauna. Cold to the bone.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 12,
    statEffects: [],
  },
  // ── Themed family loot: transit-haunts ─────────────────────────────────
  {
    id: "loot/monster-part/ticket-stub",
    kind: "item",
    name: "Ticket Stub",
    tags: ["loot:monster_part", "loot:sell_only", "loot:crafting_input", "loot:dungeon_drop"],
    description: "A crumpled transit stub that never expires. The date keeps changing.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 7,
    statEffects: [],
  },
  {
    id: "loot/monster-part/rail-splice",
    kind: "item",
    name: "Rail Splice",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A short connector from animated track. Hums at the frequency of an approaching train.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 11,
    statEffects: [],
  },

  // ── Themed family loot: fume-wraiths ─────────────────────────────────
  {
    id: "loot/monster-part/lacquer-bead",
    kind: "item",
    name: "Lacquer Bead",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description: "A hardened droplet of supernatural nail polish. The color shifts under UV.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/solvent-gland",
    kind: "item",
    name: "Solvent Gland",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "An organ-like sac from a fume wraith. The acetone inside never evaporates.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 12,
    statEffects: [],
  },

  // ── Themed family loot: rust-creepers ─────────────────────────────────
  {
    id: "loot/monster-part/oxide-flake",
    kind: "item",
    name: "Oxide Flake",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description:
      "Metallic rust chips from animated ironwork. Crumbles between fingers but cuts first.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 7,
    statEffects: [],
  },
  {
    id: "loot/monster-part/rivet-cluster",
    kind: "item",
    name: "Rivet Cluster",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "A fused mass of bolts and rivets from structural entities. Magnetically active.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 11,
    statEffects: [],
  },

  // ── Themed family loot: display-mimics ───────────────────────────────
  {
    id: "loot/monster-part/glass-tooth",
    kind: "item",
    name: "Glass Tooth",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description: "A shard of display glass shaped like a molar. Sharp on every surface.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/pawned-core",
    kind: "item",
    name: "Pawned Core",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "A dense node from the center of a display mimic. Refracts light wrong.",
    category: "loot",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 12,
    statEffects: [],
  },

  // ── Themed family loot: reel-phantoms (E-rank) ─────────────────────────
  {
    id: "loot/monster-part/celluloid-strip",
    kind: "item",
    name: "Celluloid Strip",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description:
      "A strip of film stock from an impossible reel. The frames show places that don't exist.",
    category: "loot",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 10,
    statEffects: [],
  },
  {
    id: "loot/monster-part/butter-residue",
    kind: "item",
    name: "Butter Residue",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "Congealed popcorn butter from theater entities. Warm, viscous, and faintly luminescent.",
    category: "loot",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 14,
    statEffects: [],
  },

  // ── Themed family loot: cistern-dwellers (E-rank) ─────────────────────
  {
    id: "loot/monster-part/algae-mat",
    kind: "item",
    name: "Algae Mat",
    tags: ["loot:monster_part", "loot:sell_only", "loot:auto_sell_junk", "loot:dungeon_drop"],
    description: "A thick mat of animate algae from a water tower. Writhes slowly when wet.",
    category: "loot",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 9,
    statEffects: [],
  },
  {
    id: "loot/monster-part/cistern-scale",
    kind: "item",
    name: "Cistern Scale",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "Calcified plating from water tower organisms. Rings like a bell when flicked.",
    category: "loot",
    rank: "e",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 15,
    statEffects: [],
  },

  // ── Themed family loot: astral-parasites (D-rank) ─────────────────────
  {
    id: "loot/monster-part/star-chart-shard",
    kind: "item",
    name: "Star Chart Shard",
    tags: ["loot:monster_part", "loot:sell_only", "loot:dungeon_drop"],
    description:
      "A glass fragment etched with constellations that don't match any known sky. Warm in moonlight.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 18,
    statEffects: [],
  },
  {
    id: "loot/monster-part/lens-fragment",
    kind: "item",
    name: "Lens Fragment",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A curved shard from an observatory lens. Looking through it shows a different version of the room you're in.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 22,
    statEffects: [],
  },

  // ── Themed family loot: pressure-spawn (D-rank) ───────────────────────
  {
    id: "loot/monster-part/pressure-seal",
    kind: "item",
    name: "Pressure Seal",
    tags: ["loot:monster_part", "loot:sell_only", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A gasket from pressurized pipe entities. Compressed to impossible density — heavier than lead.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 17,
    statEffects: [],
  },
  {
    id: "loot/monster-part/turbine-tooth",
    kind: "item",
    name: "Turbine Tooth",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description: "A blade from an animate turbine. Spins on flat surfaces when nobody is watching.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 24,
    statEffects: [],
  },

  // ── Themed family loot: stage-haunts (D-rank) ─────────────────────────
  {
    id: "loot/monster-part/prop-residue",
    kind: "item",
    name: "Prop Residue",
    tags: ["loot:monster_part", "loot:sell_only", "loot:dungeon_drop"],
    description:
      "Theatrical dust from animated stage equipment. Smells like greasepaint and ozone.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 16,
    statEffects: [],
  },
  {
    id: "loot/monster-part/curtain-thread",
    kind: "item",
    name: "Curtain Thread",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "Fibers from a stage curtain that moves with intent. The weave pattern changes when unobserved.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 20,
    statEffects: [],
  },

  // ── Themed family loot: lab-specimens (D-rank) ────────────────────────
  {
    id: "loot/monster-part/culture-sample",
    kind: "item",
    name: "Culture Sample",
    tags: ["loot:monster_part", "loot:sell_only", "loot:crafting_input", "loot:dungeon_drop"],
    description: "A sealed vial of rift-mutated cell culture. The cells inside are still dividing.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 19,
    statEffects: [],
  },
  {
    id: "loot/monster-part/reagent-vial",
    kind: "item",
    name: "Reagent Vial",
    tags: ["loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A stoppered vial of unknown reagent from automated lab equipment. The liquid changes color in response to nearby life.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 26,
    statEffects: [],
  },

  // ── Consumables (Prep Room products) ─────────────────────────────────
  {
    id: "consumable/rift-tonic",
    kind: "item",
    name: "Rift Tonic",
    tags: ["consumable:raid_aid", "buff:strength"],
    description:
      "A crude stimulant brewed from monster parts. Sharpens aggression for a few hours.",
    category: "consumable",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 5,
    statEffects: [],
    consumableBuff: { stat: "strength", value: 3, durationMinutes: 120 },
  },
  {
    id: "consumable/recovery-salve",
    kind: "item",
    name: "Recovery Salve",
    tags: ["consumable:raid_aid", "buff:resilience"],
    description:
      "A thick ointment that numbs pain and accelerates clotting. Apply before deployment.",
    category: "consumable",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 5,
    statEffects: [],
    consumableBuff: { stat: "resilience", value: 3, durationMinutes: 120 },
  },
  {
    id: "consumable/phase-ward",
    kind: "item",
    name: "Phase Ward",
    tags: ["consumable:raid_aid", "buff:speed"],
    description:
      "A treated bandage wrap that reacts to spatial distortion. Keeps the wearer grounded.",
    category: "consumable",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 5,
    statEffects: [],
    consumableBuff: { stat: "speed", value: 3, durationMinutes: 120 },
  },
  {
    id: "consumable/endurance-draught",
    kind: "item",
    name: "Endurance Draught",
    tags: ["consumable:raid_aid", "buff:endurance"],
    description: "A bitter drink that delays fatigue. Tastes like wet copper and works anyway.",
    category: "consumable",
    rank: "f",
    rankTone: "grounded",
    buyPrice: 0,
    sellPrice: 5,
    statEffects: [],
    consumableBuff: { stat: "endurance", value: 3, durationMinutes: 120 },
  },

  // ── Themed family loot: harbor district (D-rank) ────────────────────
  {
    id: "loot/monster-part/gravity-residue",
    kind: "item",
    name: "Gravity Residue",
    tags: ["rank:d", "loot:monster-part"],
    description:
      "A metallic smear that falls upward if you let go. Dock parasites leave it on every surface they touch.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/fold-fragment",
    kind: "item",
    name: "Fold Fragment",
    tags: ["rank:d", "loot:monster-part"],
    description:
      "A shard of packing material that shows a different warehouse when viewed from each angle.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/revenue-seal",
    kind: "item",
    name: "Revenue Seal",
    tags: ["rank:d", "loot:monster-part"],
    description:
      "An official-looking stamp that binds whatever it touches to the nearest flat surface. Still warm.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/loop-shard",
    kind: "item",
    name: "Loop Shard",
    tags: ["rank:d", "loot:monster-part"],
    description:
      "A crystallized slice of repeated time. It replays the same three seconds of a loading bay over and over.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/pressure-condensate",
    kind: "item",
    name: "Pressure Condensate",
    tags: ["rank:d", "loot:monster-part"],
    description:
      "Liquid steel that formed under impossible compression. It stays liquid at room temperature and weighs too much.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },
  {
    id: "loot/monster-part/signal-filament",
    kind: "item",
    name: "Signal Filament",
    tags: ["rank:d", "loot:monster-part"],
    description:
      "A thread of solidified harbor light. It bends around corners and hums at a frequency that makes your teeth itch.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 8,
    statEffects: [],
  },

  // ── D-rank weapons: harbor district ─────────────────────────────────
  {
    id: "weapon/dock-hook",
    kind: "item",
    name: "Dock Hook",
    tags: ["rank:d", "slot:weapon"],
    description:
      "A longshoreman's cargo hook reforged with gravity-residue alloy. Pulls targets off balance on impact.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 120,
    sellPrice: 60,
    statEffects: [
      { stat: "attack", value: 5 },
      { stat: "speed", value: 2 },
    ],
  },
  {
    id: "weapon/folded-rebar",
    kind: "item",
    name: "Folded Rebar",
    tags: ["rank:d", "slot:weapon"],
    description:
      "Structural steel bent through a spatial fold. The bend point hits harder than either end.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 140,
    sellPrice: 70,
    statEffects: [
      { stat: "attack", value: 6 },
      { stat: "endurance", value: 3 },
    ],
  },
  {
    id: "weapon/stamp-staff",
    kind: "item",
    name: "Stamp Staff",
    tags: ["rank:d", "slot:weapon"],
    description:
      "A revenue office stamp mounted on a broom handle. Anything it marks sticks to the floor for a few seconds.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 130,
    sellPrice: 65,
    statEffects: [
      { stat: "attack", value: 4 },
      { stat: "intelligence", value: 3 },
    ],
  },
  {
    id: "weapon/loop-wrench",
    kind: "item",
    name: "Loop Wrench",
    tags: ["rank:d", "slot:weapon"],
    description:
      "A pipe wrench caught in a three-second time loop. Swings land twice if you time it right.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 125,
    sellPrice: 62,
    statEffects: [
      { stat: "attack", value: 5 },
      { stat: "perception", value: 3 },
    ],
  },
  {
    id: "weapon/pressure-spike",
    kind: "item",
    name: "Pressure Spike",
    tags: ["rank:d", "slot:weapon"],
    description:
      "A ballast valve sharpened to a point. Impossibly heavy for its size and slow to swing, but the impact is final.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 160,
    sellPrice: 80,
    statEffects: [
      { stat: "attack", value: 7 },
      { stat: "speed", value: -1 },
    ],
  },
  {
    id: "weapon/signal-mace",
    kind: "item",
    name: "Signal Mace",
    tags: ["rank:d", "slot:weapon"],
    description:
      "A harbor signal lamp housing welded to a steel rod. Emits a disorienting flash on heavy strikes.",
    category: "weapon",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 135,
    sellPrice: 67,
    statEffects: [
      { stat: "attack", value: 4 },
      { stat: "perception", value: 4 },
    ],
  },

  // ── D-rank outfit overlays: harbor district ─────────────────────────
  {
    id: "outfit-overlay/gravity-weave-vest",
    kind: "item",
    name: "Gravity-Weave Vest",
    tags: ["rank:d", "slot:outfit-overlay"],
    description:
      "A dock worker's vest threaded with gravity-residue fibers. Incoming force disperses downward into the floor.",
    category: "outfit-overlay",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 150,
    sellPrice: 75,
    statEffects: [
      { stat: "defense", value: 4 },
      { stat: "endurance", value: 3 },
    ],
  },
  {
    id: "outfit-overlay/fold-stitched-jacket",
    kind: "item",
    name: "Fold-Stitched Jacket",
    tags: ["rank:d", "slot:outfit-overlay"],
    description:
      "A courier jacket with seams that pass through folded space. Hard to hit the wearer when part of them isn't technically here.",
    category: "outfit-overlay",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 145,
    sellPrice: 72,
    statEffects: [
      { stat: "defense", value: 3 },
      { stat: "speed", value: 3 },
    ],
  },
  {
    id: "outfit-overlay/audit-coat",
    kind: "item",
    name: "Audit Coat",
    tags: ["rank:d", "slot:outfit-overlay"],
    description:
      "A revenue office coat lined with stamped ward-paper. It turns incoming force into a shuddering paper-cut halo instead of a direct hit.",
    category: "outfit-overlay",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 155,
    sellPrice: 77,
    statEffects: [
      { stat: "defense", value: 4 },
      { stat: "intelligence", value: 2 },
    ],
  },

  // ── D-rank accessories: harbor district ─────────────────────────────
  {
    id: "accessory/ledger-monocle",
    kind: "item",
    name: "Ledger Monocle",
    tags: ["rank:d", "slot:accessory"],
    description:
      "A customs inspector's monocle that highlights structural weaknesses. Also works on shipping manifests.",
    category: "accessory",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 110,
    sellPrice: 55,
    statEffects: [
      { stat: "perception", value: 5 },
      { stat: "intelligence", value: 2 },
    ],
  },
  {
    id: "accessory/loop-chronometer",
    kind: "item",
    name: "Loop Chronometer",
    tags: ["rank:d", "slot:accessory"],
    description:
      "A pocket watch salvaged from a time loop. The second hand moves faster than it should, and so does the wearer.",
    category: "accessory",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 115,
    sellPrice: 57,
    statEffects: [
      { stat: "speed", value: 4 },
      { stat: "perception", value: 2 },
    ],
  },
  {
    id: "accessory/signal-prism",
    kind: "item",
    name: "Signal Prism",
    tags: ["rank:d", "slot:accessory"],
    description:
      "A harbor lens prism mounted in a brass cuff. It splits hostile movement into colored afterimages a second before impact.",
    category: "accessory",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 120,
    sellPrice: 60,
    statEffects: [
      { stat: "perception", value: 4 },
      { stat: "speed", value: 2 },
    ],
  },

  // ── D-rank consumables: harbor district ─────────────────────────────
  {
    id: "consumable/pressure-ration",
    kind: "item",
    name: "Pressure Ration",
    tags: ["rank:d", "consumable:buff"],
    description:
      "A compressed meal bar from the ballast crews. Dense enough to chip a tooth and filling enough to last a full shift.",
    category: "consumable",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 0,
    statEffects: [],
    consumableBuff: { stat: "endurance", value: 4, durationMinutes: 30 },
  },
  {
    id: "consumable/signal-tonic",
    kind: "item",
    name: "Signal Tonic",
    tags: ["rank:d", "consumable:buff"],
    description:
      "A bitter tincture brewed from dissolved signal filaments. Sharpens awareness but makes your fillings hum.",
    category: "consumable",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 0,
    statEffects: [],
    consumableBuff: { stat: "perception", value: 4, durationMinutes: 30 },
  },
  // ── District rare materials ────────────────────────────────────────────
  {
    id: "loot/rare-material/rail-slag",
    kind: "item",
    name: "Rail Slag",
    tags: ["rank:d", "material:rail-slag"],
    description:
      "Rift-fused rail residue from the Queens switchyard. Required for precision-forge weapon and armor recipes.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 40,
    statEffects: [],
  },
  {
    id: "loot/rare-material/overpass-aggregate",
    kind: "item",
    name: "Overpass Aggregate",
    tags: ["rank:d", "material:overpass-aggregate"],
    description:
      "Rift-softened highway composite from the Bronx overpass. Required for heavy structural craft recipes.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 40,
    statEffects: [],
  },
  {
    id: "loot/rare-material/brine-crystal",
    kind: "item",
    name: "Brine Crystal",
    tags: ["rank:d", "material:brine-crystal"],
    description:
      "Saline crystal formations from the Red Hook waterfront rifts. Required for maritime-grade craft recipes.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 40,
    statEffects: [],
  },
  {
    id: "loot/rare-material/conduit-filament",
    kind: "item",
    name: "Conduit Filament",
    tags: ["rank:d", "material:conduit-filament"],
    description:
      "Rift-charged copper filament from the Harlem substation. Required for electrically-tuned craft recipes.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 40,
    statEffects: [],
  },
  // ── D-rank encounter-expansion monster parts ───────────────────────────
  {
    id: "loot/monster-part/customs-seal",
    kind: "item",
    name: "Customs Seal",
    tags: ["rank:d"],
    description: "A pneumatic tube seal still humming with processing intent.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 12,
    statEffects: [],
  },
  {
    id: "loot/monster-part/import-stamp",
    kind: "item",
    name: "Import Stamp",
    tags: ["rank:d"],
    description: "A customs stamp head that still leaves marks on anything it touches.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 14,
    statEffects: [],
  },
  {
    id: "loot/monster-part/switch-plate",
    kind: "item",
    name: "Switch Plate",
    tags: ["rank:d"],
    description: "A rail switch plate that still clicks when nothing is touching it.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 14,
    statEffects: [],
  },
  {
    id: "loot/monster-part/winding-coil",
    kind: "item",
    name: "Winding Coil",
    tags: ["rank:d"],
    description: "A transformer coil segment still carrying residual charge.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 12,
    statEffects: [],
  },
  {
    id: "loot/monster-part/insulator-shard",
    kind: "item",
    name: "Insulator Shard",
    tags: ["rank:d"],
    description: "A porcelain insulator fragment that repels current and curiosity.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 14,
    statEffects: [],
  },
  {
    id: "loot/monster-part/tar-bead",
    kind: "item",
    name: "Tar Bead",
    tags: ["rank:d"],
    description: "A hardened bead of rift-softened asphalt that remembers being road.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 12,
    statEffects: [],
  },
  {
    id: "loot/monster-part/reflector-chip",
    kind: "item",
    name: "Reflector Chip",
    tags: ["rank:d"],
    description: "A road reflector chip that still catches light from a sun that isn't there.",
    category: "loot",
    rank: "d",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 14,
    statEffects: [],
  },

  // ── C-rank monster parts (skyscraper endgame entry) ──────────────────
  {
    id: "loot/monster-part/ticker-ribbon",
    kind: "item",
    name: "Ticker Ribbon",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A coiled length of ticker tape that still prints rising quotes in a failed language. The tape is warm to the touch.",
    category: "loot",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 22,
    statEffects: [],
  },
  {
    id: "loot/monster-part/exchange-coin",
    kind: "item",
    name: "Exchange Coin",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A commemorative trading-floor coin minted for a market that never opened. Its edge is machined to a wavelength you can feel in your teeth.",
    category: "loot",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 32,
    statEffects: [],
  },
  {
    id: "loot/monster-part/residence-key",
    kind: "item",
    name: "Residence Key",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A brass apartment key that fits every door on a floor that does not exist. The fob hums when pocketed.",
    category: "loot",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 24,
    statEffects: [],
  },
  {
    id: "loot/monster-part/penthouse-velvet",
    kind: "item",
    name: "Penthouse Velvet",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A scrap of residential-tower drapery that resists being creased. Catches dust from rooms you have not entered.",
    category: "loot",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 34,
    statEffects: [],
  },
  {
    id: "loot/monster-part/corridor-glass",
    kind: "item",
    name: "Corridor Glass",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A slab of skybridge glazing that reflects a corridor without an ending. The reflection shows fewer commuters each time.",
    category: "loot",
    rank: "c",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 22,
    statEffects: [],
  },
  {
    id: "loot/monster-part/corporate-lanyard",
    kind: "item",
    name: "Corporate Lanyard",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A security lanyard whose photograph updates when no one is looking. The badge still unlocks doors — but only in the wrong building.",
    category: "loot",
    rank: "c",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 30,
    statEffects: [],
  },
  {
    id: "loot/monster-part/express-token",
    kind: "item",
    name: "Express Token",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A subway token stamped with a destination code that has not existed in seven decades. Turnstiles still accept it.",
    category: "loot",
    rank: "c",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 24,
    statEffects: [],
  },
  {
    id: "loot/monster-part/platform-rail",
    kind: "item",
    name: "Platform Rail",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A cross-section of subway platform edging that rings at a pitch no rolling stock has produced. Conducts heat from a nearby express.",
    category: "loot",
    rank: "c",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 32,
    statEffects: [],
  },
  {
    id: "loot/monster-part/docent-label",
    kind: "item",
    name: "Docent Label",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "An exhibit placard whose caption updates in real time. Each update is a fraction more accurate about its bearer.",
    category: "loot",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 24,
    statEffects: [],
  },
  {
    id: "loot/monster-part/gallery-gold-leaf",
    kind: "item",
    name: "Gallery Gold Leaf",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A gilded flake from the frame of a painting that is no longer on any wall. The gold is slightly too warm to have been merely pressed.",
    category: "loot",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 34,
    statEffects: [],
  },
  {
    id: "loot/monster-part/observation-lens",
    kind: "item",
    name: "Observation Lens",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A lens shard from the coin-operated binoculars. Looking through it shows rooms beneath the deck that are not on any plan.",
    category: "loot",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 24,
    statEffects: [],
  },
  {
    id: "loot/monster-part/altitude-resin",
    kind: "item",
    name: "Altitude Resin",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A semi-solid slick from the observation deck's upper vents. Cold even in your palm. Smells faintly of rain that has not yet fallen.",
    category: "loot",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 32,
    statEffects: [],
  },
  {
    id: "loot/monster-part/spire-residue",
    kind: "item",
    name: "Spire Residue",
    tags: ["rank:c", "loot:monster_part", "loot:crafting_input", "loot:dungeon_drop"],
    description:
      "A cross-site C-rank residue that spontaneously accumulates near high-altitude rifts. Needed to stabilize most C-rank craft recipes.",
    category: "loot",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 40,
    statEffects: [],
  },

  // ── C-rank weapons (skyscraper endgame entry) ────────────────────────
  {
    id: "weapon/market-scythe",
    kind: "item",
    name: "Market Scythe",
    tags: ["rank:c", "slot:weapon"],
    description:
      "A reforged ticker-tape ribbon spun into a long-handled curved blade. The edge hums when it detects a position worth closing.",
    category: "weapon",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 120,
    statEffects: [
      { stat: "strength", value: 8 },
      { stat: "intelligence", value: 4 },
      { stat: "speed", value: 2 },
    ],
  },
  {
    id: "weapon/concierge-baton",
    kind: "item",
    name: "Concierge Baton",
    tags: ["rank:c", "slot:weapon"],
    description:
      "A brass service baton cored with residence-key alloy. A single strike can restate a room's tenancy rights against its current occupants.",
    category: "weapon",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 115,
    statEffects: [
      { stat: "strength", value: 7 },
      { stat: "perception", value: 4 },
      { stat: "resilience", value: 3 },
    ],
  },
  {
    id: "weapon/skybridge-rail",
    kind: "item",
    name: "Skybridge Rail",
    tags: ["rank:c", "slot:weapon"],
    description:
      "A severed length of skybridge handrail, still mounted to its glazed stanchion. The reflective edge keeps showing the wielder a commuter ghost they are about to hit.",
    category: "weapon",
    rank: "c",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 108,
    statEffects: [
      { stat: "strength", value: 9 },
      { stat: "endurance", value: 3 },
      { stat: "speed", value: -1 },
    ],
  },
  {
    id: "weapon/express-pike",
    kind: "item",
    name: "Express Pike",
    tags: ["rank:c", "slot:weapon"],
    description:
      "A platform-rail section reshaped into a two-handed thrusting pike. A committed strike resonates with the interchange's trains and pulls them slightly sideways.",
    category: "weapon",
    rank: "c",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 112,
    statEffects: [
      { stat: "strength", value: 8 },
      { stat: "perception", value: 4 },
      { stat: "speed", value: 2 },
    ],
  },
  {
    id: "weapon/curator-stylus",
    kind: "item",
    name: "Curator Stylus",
    tags: ["rank:c", "slot:weapon"],
    description:
      "A gilded laser-pointer rebuilt around a gallery-gold-leaf lens. Beam carries enough narrative weight to shear through whatever the docent's placard now describes.",
    category: "weapon",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 118,
    statEffects: [
      { stat: "intelligence", value: 7 },
      { stat: "perception", value: 5 },
      { stat: "speed", value: 2 },
    ],
  },
  {
    id: "weapon/altitude-spike",
    kind: "item",
    name: "Altitude Spike",
    tags: ["rank:c", "slot:weapon"],
    description:
      "A spotter's spike forged from the binoculars' inner barrel. The point locks onto a target's updraft signature and does not release until the altitude equalizes.",
    category: "weapon",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 110,
    statEffects: [
      { stat: "strength", value: 7 },
      { stat: "perception", value: 5 },
      { stat: "speed", value: 3 },
    ],
  },

  // ── C-rank outfit overlays ──────────────────────────────────────────
  {
    id: "outfit-overlay/exchange-bespoke",
    kind: "item",
    name: "Exchange Bespoke Suit",
    tags: ["rank:c", "slot:outfit-overlay"],
    description:
      "A tailored trading-floor suit lined with pressed ticker tape. Dampens incoming concussive shouts and redistributes the impact along its pinstripes.",
    category: "outfit-overlay",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 120,
    statEffects: [
      { stat: "resilience", value: 7 },
      { stat: "perception", value: 3 },
      { stat: "speed", value: 2 },
    ],
  },
  {
    id: "outfit-overlay/residence-longcoat",
    kind: "item",
    name: "Residence Longcoat",
    tags: ["rank:c", "slot:outfit-overlay"],
    description:
      "A concierge-cut greatcoat woven through with penthouse velvet. The wearer reads as an authorized resident in any recursive unit on the floor.",
    category: "outfit-overlay",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 124,
    statEffects: [
      { stat: "resilience", value: 8 },
      { stat: "endurance", value: 3 },
      { stat: "intelligence", value: 2 },
    ],
  },
  {
    id: "outfit-overlay/skybridge-carrier",
    kind: "item",
    name: "Skybridge Plate Carrier",
    tags: ["rank:c", "slot:outfit-overlay"],
    description:
      "A heavy plate carrier reinforced with corridor-glass panels. Incoming force is routed through the panels and arrives somewhere else on the bridge.",
    category: "outfit-overlay",
    rank: "c",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 118,
    statEffects: [
      { stat: "resilience", value: 9 },
      { stat: "endurance", value: 3 },
      { stat: "speed", value: -1 },
    ],
  },
  {
    id: "outfit-overlay/gallery-smock",
    kind: "item",
    name: "Gallery Conservator Smock",
    tags: ["rank:c", "slot:outfit-overlay"],
    description:
      "A curator's field smock sewn from docent labels and archival linen. Whatever the placard says is true of the wearer, within reason.",
    category: "outfit-overlay",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 115,
    statEffects: [
      { stat: "resilience", value: 6 },
      { stat: "intelligence", value: 4 },
      { stat: "perception", value: 3 },
    ],
  },

  // ── C-rank accessories ──────────────────────────────────────────────
  {
    id: "accessory/margin-cufflinks",
    kind: "item",
    name: "Margin Cufflinks",
    tags: ["rank:c", "slot:accessory"],
    description:
      "A pair of exchange-coin cufflinks that vibrate when the wearer is in range of a loss. Slight motion helps you cover the gap.",
    category: "accessory",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 92,
    statEffects: [
      { stat: "perception", value: 5 },
      { stat: "speed", value: 3 },
      { stat: "resilience", value: 2 },
    ],
  },
  {
    id: "accessory/residence-keyring",
    kind: "item",
    name: "Residence Keyring",
    tags: ["rank:c", "slot:accessory"],
    description:
      "A brass keyring with one key for every unit on a floor. Each key lets you decline a structural threat's move-in offer.",
    category: "accessory",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 96,
    statEffects: [
      { stat: "resilience", value: 4 },
      { stat: "endurance", value: 3 },
      { stat: "perception", value: 3 },
    ],
  },
  {
    id: "accessory/corridor-goggles",
    kind: "item",
    name: "Corridor Goggles",
    tags: ["rank:c", "slot:accessory"],
    description:
      "Tinted glass goggles with corridor-glass reflective treatment. Shows the wearer the commuter line three seconds before it arrives.",
    category: "accessory",
    rank: "c",
    rankTone: "heightened",
    buyPrice: 0,
    sellPrice: 88,
    statEffects: [
      { stat: "perception", value: 6 },
      { stat: "speed", value: 3 },
    ],
  },
  {
    id: "accessory/altitude-earpiece",
    kind: "item",
    name: "Altitude Earpiece",
    tags: ["rank:c", "slot:accessory"],
    description:
      "An in-ear receiver calibrated to the observation deck's pressure band. Tells the wearer which direction is actually up when the building forgets.",
    category: "accessory",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 90,
    statEffects: [
      { stat: "perception", value: 5 },
      { stat: "speed", value: 3 },
      { stat: "resilience", value: 2 },
    ],
  },

  // ── C-rank consumables ───────────────────────────────────────────────
  {
    id: "consumable/altitude-tonic",
    kind: "item",
    name: "Altitude Tonic",
    tags: ["rank:c", "consumable:raid_aid", "buff:perception"],
    description:
      "A bitter infusion of altitude resin and observation lens powder. Holds the wearer's bearings steady when the site rotates the sky.",
    category: "consumable",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 18,
    statEffects: [],
    consumableBuff: { stat: "perception", value: 6, durationMinutes: 120 },
  },
  {
    id: "consumable/market-stimulant",
    kind: "item",
    name: "Market Stimulant",
    tags: ["rank:c", "consumable:raid_aid", "buff:intelligence"],
    description:
      "A trading-floor adrenal brew that pushes the wearer into a short-term edge case where hostile abilities mis-clear.",
    category: "consumable",
    rank: "c",
    rankTone: "surreal",
    buyPrice: 0,
    sellPrice: 20,
    statEffects: [],
    consumableBuff: { stat: "intelligence", value: 6, durationMinutes: 120 },
  },
] satisfies readonly ItemTemplate[];

// ── Prep Recipes ──────────────────────────────────────────────────────
export const prepRecipeTemplates: readonly PrepRecipeTemplate[] = [
  {
    id: "prep-recipe/rift-tonic",
    name: "Rift Tonic",
    description: "Combine fangs and ichor into a crude stimulant.",
    inputs: [
      { itemId: "loot/monster-part/fang", quantity: 2 },
      { itemId: "loot/monster-part/ichor", quantity: 1 },
    ],
    outputItemId: "consumable/rift-tonic",
    outputQuantity: 1,
    requiredRoomTag: "ops:staging",
  },
  {
    id: "prep-recipe/recovery-salve",
    name: "Recovery Salve",
    description: "Grind carapace fragments into a protective ointment.",
    inputs: [
      { itemId: "loot/monster-part/carapace", quantity: 2 },
      { itemId: "loot/monster-part/sinew", quantity: 1 },
    ],
    outputItemId: "consumable/recovery-salve",
    outputQuantity: 1,
    requiredRoomTag: "ops:staging",
  },
  {
    id: "prep-recipe/phase-ward",
    name: "Phase Ward",
    description: "Treat bandages with crystal-eye extract for spatial stability.",
    inputs: [
      { itemId: "loot/monster-part/crystal-eye", quantity: 1 },
      { itemId: "loot/monster-part/bone-shard", quantity: 2 },
    ],
    outputItemId: "consumable/phase-ward",
    outputQuantity: 1,
    requiredRoomTag: "ops:staging",
  },
  {
    id: "prep-recipe/endurance-draught",
    name: "Endurance Draught",
    description: "Boil sinew and ichor into a fatigue-delaying drink.",
    inputs: [
      { itemId: "loot/monster-part/sinew", quantity: 2 },
      { itemId: "loot/monster-part/ichor", quantity: 2 },
    ],
    outputItemId: "consumable/endurance-draught",
    outputQuantity: 1,
    requiredRoomTag: "ops:staging",
  },
  {
    id: "prep-recipe/pressure-ration",
    name: "Pressure Ration",
    description: "Compress condensate and bone shards into a dense meal bar.",
    inputs: [
      { itemId: "loot/monster-part/pressure-condensate", quantity: 2 },
      { itemId: "loot/monster-part/bone-shard", quantity: 1 },
    ],
    outputItemId: "consumable/pressure-ration",
    outputQuantity: 1,
    requiredRoomTag: "ops:staging",
  },
  {
    id: "prep-recipe/signal-tonic",
    name: "Signal Tonic",
    description: "Dissolve signal filaments in crystal-eye extract for a perception tincture.",
    inputs: [
      { itemId: "loot/monster-part/signal-filament", quantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", quantity: 1 },
    ],
    outputItemId: "consumable/signal-tonic",
    outputQuantity: 1,
    requiredRoomTag: "ops:staging",
  },
];

export const dropTables = [
  {
    id: "drop-table/dungeon-f-regular",
    entries: [
      { itemId: "loot/monster-part/fang", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 25, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/bone-shard", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/dungeon-f-elite",
    entries: [
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/bone-shard", weight: 20, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/dungeon-f-boss",
    entries: [
      { itemId: "loot/monster-part/void-residue", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/pipe-wrench", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "weapon/kitchen-knife", weight: 3, minQuantity: 1, maxQuantity: 1 },
      { itemId: "weapon/crowbar", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "weapon/box-cutter", weight: 3, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/padded-jacket", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/work-coveralls", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/comm-earpiece", weight: 1, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/wrist-brace", weight: 1, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 20, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  // ── Per-family regular tables ────────────────────────────────────────
  {
    id: "drop-table/tunnel-crawlers-regular",
    entries: [
      { itemId: "loot/monster-part/drain-sludge", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/pipe-scale", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/concrete-sentinels-regular",
    entries: [
      { itemId: "loot/monster-part/ramp-gravel", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bollard-core", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bone-shard", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/chalk-swarms-regular",
    entries: [
      { itemId: "loot/monster-part/chalk-filament", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/lesson-fragment", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/sinew", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/vine-constructs-regular",
    entries: [
      { itemId: "loot/monster-part/root-fiber", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/pollen-sac", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/suds-constructs-regular",
    entries: [
      { itemId: "loot/monster-part/lint-clump", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/detergent-crystal", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/ichor", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/cellar-dwellers-regular",
    entries: [
      { itemId: "loot/monster-part/radiator-ash", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/basement-mold", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bone-shard", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/court-echoes-regular",
    entries: [
      { itemId: "loot/monster-part/echo-resin", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/rubber-fragment", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/sinew", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/freezer-fauna-regular",
    entries: [
      { itemId: "loot/monster-part/frost-shard", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/preserved-marrow", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },

  // ── Per-family elite tables ─────────────────────────────────────────
  {
    id: "drop-table/tunnel-crawlers-elite",
    entries: [
      { itemId: "loot/monster-part/pipe-scale", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/drain-sludge", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/concrete-sentinels-elite",
    entries: [
      { itemId: "loot/monster-part/bollard-core", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/ramp-gravel", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/chalk-swarms-elite",
    entries: [
      { itemId: "loot/monster-part/lesson-fragment", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/chalk-filament", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/vine-constructs-elite",
    entries: [
      { itemId: "loot/monster-part/pollen-sac", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/root-fiber", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/suds-constructs-elite",
    entries: [
      { itemId: "loot/monster-part/detergent-crystal", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/lint-clump", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/cellar-dwellers-elite",
    entries: [
      { itemId: "loot/monster-part/basement-mold", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/radiator-ash", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/court-echoes-elite",
    entries: [
      { itemId: "loot/monster-part/rubber-fragment", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/echo-resin", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/freezer-fauna-elite",
    entries: [
      { itemId: "loot/monster-part/preserved-marrow", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/frost-shard", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },

  // ── Per-boss tables ─────────────────────────────────────────────────
  {
    id: "drop-table/tunneler-brood-mother",
    entries: [
      { itemId: "loot/monster-part/pipe-scale", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/drain-sludge", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/pipe-wrench", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/padded-jacket", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/sewer-warden",
    entries: [
      { itemId: "loot/monster-part/bollard-core", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/ramp-gravel", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/crowbar", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/work-coveralls", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/phantom-stalker",
    entries: [
      { itemId: "loot/monster-part/lesson-fragment", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/chalk-filament", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/kitchen-knife", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/comm-earpiece", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/the-curator",
    entries: [
      { itemId: "loot/monster-part/pollen-sac", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/root-fiber", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/box-cutter", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/leather-vest", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/the-attendant",
    entries: [
      { itemId: "loot/monster-part/detergent-crystal", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/lint-clump", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/pipe-wrench", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/work-coveralls", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/the-super",
    entries: [
      { itemId: "loot/monster-part/basement-mold", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/radiator-ash", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/crowbar", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/wrist-brace", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/the-referee",
    entries: [
      { itemId: "loot/monster-part/rubber-fragment", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/echo-resin", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/baseball-bat", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/tactical-scarf", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/the-stockkeeper",
    entries: [
      { itemId: "loot/monster-part/preserved-marrow", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/frost-shard", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/kitchen-knife", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/padded-jacket", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── Family tables: transit-haunts ─────────────────────────────────────
  {
    id: "drop-table/transit-haunts-regular",
    entries: [
      { itemId: "loot/monster-part/ticket-stub", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/rail-splice", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bone-shard", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/transit-haunts-elite",
    entries: [
      { itemId: "loot/monster-part/rail-splice", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/ticket-stub", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-dispatcher",
    entries: [
      { itemId: "loot/monster-part/rail-splice", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/ticket-stub", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/crowbar", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/work-coveralls", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── Family tables: fume-wraiths ──────────────────────────────────────
  {
    id: "drop-table/fume-wraiths-regular",
    entries: [
      { itemId: "loot/monster-part/lacquer-bead", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/solvent-gland", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 25, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/sinew", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 15, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/fume-wraiths-elite",
    entries: [
      { itemId: "loot/monster-part/solvent-gland", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/lacquer-bead", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-manicurist",
    entries: [
      { itemId: "loot/monster-part/solvent-gland", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/lacquer-bead", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/box-cutter", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/comm-earpiece", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/ichor", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── Family tables: rust-creepers ──────────────────────────────────────
  {
    id: "drop-table/rust-creepers-regular",
    entries: [
      { itemId: "loot/monster-part/oxide-flake", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/rivet-cluster", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/rust-creepers-elite",
    entries: [
      { itemId: "loot/monster-part/rivet-cluster", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/oxide-flake", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-inspector",
    entries: [
      { itemId: "loot/monster-part/rivet-cluster", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/oxide-flake", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/pipe-wrench", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/padded-jacket", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/carapace", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── Family tables: display-mimics ─────────────────────────────────────
  {
    id: "drop-table/display-mimics-regular",
    entries: [
      { itemId: "loot/monster-part/glass-tooth", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/pawned-core", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/sinew", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/display-mimics-elite",
    entries: [
      { itemId: "loot/monster-part/pawned-core", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/glass-tooth", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-appraiser",
    entries: [
      { itemId: "loot/monster-part/pawned-core", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/glass-tooth", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/kitchen-knife", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/wrist-brace", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  // ── E-rank family tables: reel-phantoms ─────────────────────────────
  {
    id: "drop-table/reel-phantoms-regular",
    entries: [
      { itemId: "loot/monster-part/celluloid-strip", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/butter-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/sinew", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/reel-phantoms-elite",
    entries: [
      { itemId: "loot/monster-part/butter-residue", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/celluloid-strip", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-projectionist",
    entries: [
      { itemId: "loot/monster-part/butter-residue", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/celluloid-strip", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/stun-baton", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/reinforced-hoodie", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── E-rank family tables: cistern-dwellers ────────────────────────────
  {
    id: "drop-table/cistern-dwellers-regular",
    entries: [
      { itemId: "loot/monster-part/algae-mat", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/cistern-scale", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/cistern-dwellers-elite",
    entries: [
      { itemId: "loot/monster-part/cistern-scale", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/algae-mat", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-valve-master",
    entries: [
      { itemId: "loot/monster-part/cistern-scale", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/algae-mat", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/rebar-spike", weight: 8, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/insulated-parka", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── D-rank base tables ─────────────────────────────────────────────
  {
    id: "drop-table/dungeon-d-regular",
    entries: [
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/bone-shard", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/dungeon-d-elite",
    entries: [
      { itemId: "loot/monster-part/void-residue", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 15, minQuantity: 2, maxQuantity: 3 },
      { itemId: "loot/monster-part/bone-shard", weight: 15, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/dungeon-d-boss",
    entries: [
      { itemId: "loot/monster-part/void-residue", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 2, maxQuantity: 3 },
      { itemId: "weapon/reinforced-bat", weight: 3, minQuantity: 1, maxQuantity: 1 },
      { itemId: "weapon/rift-scored-blade", weight: 3, minQuantity: 1, maxQuantity: 1 },
      { itemId: "weapon/pulse-baton", weight: 3, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/ballistic-vest", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/rift-lined-jacket", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/utility-belt", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/rift-compass", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/carapace", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/fang", weight: 13, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── D-rank family tables: astral-parasites ────────────────────────────
  {
    id: "drop-table/astral-parasites-regular",
    entries: [
      { itemId: "loot/monster-part/star-chart-shard", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/lens-fragment", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/fang", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/astral-parasites-elite",
    entries: [
      { itemId: "loot/monster-part/lens-fragment", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/star-chart-shard", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-astronomer",
    entries: [
      { itemId: "loot/monster-part/lens-fragment", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/star-chart-shard", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/rare-material/brine-crystal", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "weapon/focused-carbine", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/rift-compass", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── D-rank family tables: pressure-spawn ──────────────────────────────
  {
    id: "drop-table/pressure-spawn-regular",
    entries: [
      { itemId: "loot/monster-part/pressure-seal", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/turbine-tooth", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/pressure-spawn-elite",
    entries: [
      { itemId: "loot/monster-part/turbine-tooth", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/pressure-seal", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-engineer",
    entries: [
      { itemId: "loot/monster-part/turbine-tooth", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/pressure-seal", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/rare-material/brine-crystal", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "weapon/breach-hammer", weight: 5, minQuantity: 1, maxQuantity: 1 },
      {
        itemId: "outfit-overlay/composite-plate-carrier",
        weight: 5,
        minQuantity: 1,
        maxQuantity: 1,
      },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/bone-shard", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── D-rank family tables: stage-haunts ────────────────────────────────
  {
    id: "drop-table/stage-haunts-regular",
    entries: [
      { itemId: "loot/monster-part/prop-residue", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/curtain-thread", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bone-shard", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/stage-haunts-elite",
    entries: [
      { itemId: "loot/monster-part/curtain-thread", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/prop-residue", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/sinew", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-director",
    entries: [
      { itemId: "loot/monster-part/curtain-thread", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/prop-residue", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/rare-material/conduit-filament", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "weapon/pulse-baton", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/dampening-earbuds", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/sinew", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── D-rank family tables: lab-specimens ───────────────────────────────
  {
    id: "drop-table/lab-specimens-regular",
    entries: [
      { itemId: "loot/monster-part/culture-sample", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/reagent-vial", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 20, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/lab-specimens-elite",
    entries: [
      { itemId: "loot/monster-part/reagent-vial", weight: 25, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/culture-sample", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/ichor", weight: 25, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-researcher",
    entries: [
      { itemId: "loot/monster-part/reagent-vial", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/culture-sample", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/rare-material/rail-slag", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "weapon/rift-scored-blade", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/signal-dampening-coat", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/ichor", weight: 10, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── D-rank family tables: harbor district ─────────────────────────────
  {
    id: "drop-table/gravity-lice-regular",
    entries: [
      { itemId: "loot/monster-part/gravity-residue", weight: 5, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 3, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bone-shard", weight: 2, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/fold-walkers-regular",
    entries: [
      { itemId: "loot/monster-part/fold-fragment", weight: 5, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/prop-residue", weight: 3, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/curtain-thread", weight: 2, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/ledger-wraiths-regular",
    entries: [
      { itemId: "loot/monster-part/revenue-seal", weight: 5, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 3, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 2, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/loop-echoes-regular",
    entries: [
      { itemId: "loot/monster-part/loop-shard", weight: 5, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bone-shard", weight: 3, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 2, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/pressure-hulks-regular",
    entries: [
      {
        itemId: "loot/monster-part/pressure-condensate",
        weight: 5,
        minQuantity: 1,
        maxQuantity: 2,
      },
      { itemId: "loot/monster-part/pressure-seal", weight: 3, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/turbine-tooth", weight: 2, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/signal-phantoms-regular",
    entries: [
      {
        itemId: "loot/monster-part/signal-filament",
        weight: 5,
        minQuantity: 1,
        maxQuantity: 2,
      },
      { itemId: "loot/monster-part/crystal-eye", weight: 3, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 2, minQuantity: 1, maxQuantity: 2 },
    ],
  },

  // ── D-rank boss tables: harbor district ───────────────────────────────
  {
    id: "drop-table/the-dockmaster-boss",
    entries: [
      { itemId: "loot/monster-part/gravity-residue", weight: 4, minQuantity: 2, maxQuantity: 4 },
      { itemId: "weapon/dock-hook", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 3, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-manifest-clerk-boss",
    entries: [
      { itemId: "loot/monster-part/fold-fragment", weight: 4, minQuantity: 2, maxQuantity: 4 },
      { itemId: "weapon/folded-rebar", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/prop-residue", weight: 3, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-revenue-agent-boss",
    entries: [
      { itemId: "loot/monster-part/revenue-seal", weight: 4, minQuantity: 2, maxQuantity: 4 },
      { itemId: "weapon/stamp-staff", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/audit-coat", weight: 1, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 3, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-lift-operator-boss",
    entries: [
      { itemId: "loot/monster-part/loop-shard", weight: 4, minQuantity: 2, maxQuantity: 4 },
      { itemId: "weapon/loop-wrench", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 3, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-ballast-master-boss",
    entries: [
      {
        itemId: "loot/monster-part/pressure-condensate",
        weight: 4,
        minQuantity: 2,
        maxQuantity: 4,
      },
      { itemId: "weapon/pressure-spike", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/pressure-seal", weight: 3, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/the-signalman-boss",
    entries: [
      {
        itemId: "loot/monster-part/signal-filament",
        weight: 4,
        minQuantity: 2,
        maxQuantity: 4,
      },
      { itemId: "weapon/signal-mace", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/signal-prism", weight: 1, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/crystal-eye", weight: 3, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  // ── D-rank encounter-expansion family tables ──────────────────────────
  {
    id: "drop-table/customs-sentries-regular",
    entries: [
      { itemId: "loot/monster-part/customs-seal", weight: 5, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/rivet-cluster", weight: 3, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 3, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 2, minQuantity: 1, maxQuantity: 2 },
    ],
  },
  {
    id: "drop-table/customs-sentries-elite",
    entries: [
      { itemId: "loot/monster-part/import-stamp", weight: 4, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bollard-core", weight: 2, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/customs-seal", weight: 3, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/void-residue", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/rare-material/brine-crystal", weight: 1, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  {
    id: "drop-table/rail-grinders-regular",
    entries: [
      { itemId: "loot/monster-part/rail-splice", weight: 5, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/ticket-stub", weight: 4, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bone-shard", weight: 3, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 2, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  {
    id: "drop-table/rail-grinders-elite",
    entries: [
      { itemId: "loot/monster-part/switch-plate", weight: 4, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/ticket-stub", weight: 3, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/rail-splice", weight: 3, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/threat-gland", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/rare-material/rail-slag", weight: 1, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  {
    id: "drop-table/arc-crawlers-regular",
    entries: [
      { itemId: "loot/monster-part/winding-coil", weight: 5, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 3, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 2, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  {
    id: "drop-table/arc-crawlers-elite",
    entries: [
      { itemId: "loot/monster-part/insulator-shard", weight: 4, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/winding-coil", weight: 3, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/rare-material/conduit-filament", weight: 1, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  {
    id: "drop-table/asphalt-shapers-regular",
    entries: [
      { itemId: "loot/monster-part/tar-bead", weight: 5, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/ramp-gravel", weight: 4, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/bone-shard", weight: 3, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/carapace", weight: 2, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  {
    id: "drop-table/asphalt-shapers-elite",
    entries: [
      { itemId: "loot/monster-part/reflector-chip", weight: 4, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/ramp-gravel", weight: 3, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/bollard-core", weight: 2, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/tar-bead", weight: 3, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/carapace", weight: 2, minQuantity: 1, maxQuantity: 1 },
      {
        itemId: "loot/rare-material/overpass-aggregate",
        weight: 1,
        minQuantity: 1,
        maxQuantity: 1,
      },
    ],
  },
  // ── D-rank encounter-expansion boss tables ────────────────────────────
  {
    id: "drop-table/the-excise-officer",
    entries: [
      { itemId: "loot/monster-part/import-stamp", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/customs-seal", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/rare-material/brine-crystal", weight: 8, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/breach-hammer", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/trauma-kit", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/fang", weight: 9, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/the-yardmaster",
    entries: [
      { itemId: "loot/monster-part/switch-plate", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/rail-splice", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/rare-material/rail-slag", weight: 8, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/focused-carbine", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/rift-compass", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/bone-shard", weight: 9, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/the-regulator",
    entries: [
      { itemId: "loot/monster-part/insulator-shard", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/winding-coil", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/rare-material/conduit-filament", weight: 8, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/pulse-baton", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/field-medic-coat", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/ichor", weight: 9, minQuantity: 2, maxQuantity: 4 },
    ],
  },
  {
    id: "drop-table/the-surveyor",
    entries: [
      { itemId: "loot/monster-part/reflector-chip", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/tar-bead", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      {
        itemId: "loot/rare-material/overpass-aggregate",
        weight: 8,
        minQuantity: 1,
        maxQuantity: 2,
      },
      {
        itemId: "outfit-overlay/composite-plate-carrier",
        weight: 4,
        minQuantity: 1,
        maxQuantity: 1,
      },
      { itemId: "accessory/reinforced-gloves", weight: 4, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/carapace", weight: 9, minQuantity: 2, maxQuantity: 4 },
    ],
  },

  // ── C-rank family tables: ticker-phantoms ─────────────────────────────
  {
    id: "drop-table/ticker-phantoms-regular",
    entries: [
      { itemId: "loot/monster-part/ticker-ribbon", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/exchange-coin", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/fang", weight: 10, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/ticker-phantoms-elite",
    entries: [
      { itemId: "loot/monster-part/exchange-coin", weight: 25, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/ticker-ribbon", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },

  // ── C-rank family tables: penthouse-sirens ───────────────────────────
  {
    id: "drop-table/penthouse-sirens-regular",
    entries: [
      { itemId: "loot/monster-part/residence-key", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/penthouse-velvet", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/sinew", weight: 10, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/penthouse-sirens-elite",
    entries: [
      { itemId: "loot/monster-part/penthouse-velvet", weight: 25, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/residence-key", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },

  // ── C-rank family tables: skybridge-commuters ──────────────────────
  {
    id: "drop-table/skybridge-commuters-regular",
    entries: [
      { itemId: "loot/monster-part/corridor-glass", weight: 30, minQuantity: 1, maxQuantity: 2 },
      {
        itemId: "loot/monster-part/corporate-lanyard",
        weight: 20,
        minQuantity: 1,
        maxQuantity: 2,
      },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 10, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/skybridge-commuters-elite",
    entries: [
      {
        itemId: "loot/monster-part/corporate-lanyard",
        weight: 25,
        minQuantity: 1,
        maxQuantity: 3,
      },
      { itemId: "loot/monster-part/corridor-glass", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },

  // ── C-rank family tables: interchange-commuters ────────────────────
  {
    id: "drop-table/interchange-commuters-regular",
    entries: [
      { itemId: "loot/monster-part/express-token", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/platform-rail", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/carapace", weight: 10, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/interchange-commuters-elite",
    entries: [
      { itemId: "loot/monster-part/platform-rail", weight: 25, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/express-token", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },

  // ── C-rank family tables: gallery-wraiths ──────────────────────────
  {
    id: "drop-table/gallery-wraiths-regular",
    entries: [
      { itemId: "loot/monster-part/docent-label", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/gallery-gold-leaf", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/bone-shard", weight: 10, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/gallery-wraiths-elite",
    entries: [
      { itemId: "loot/monster-part/gallery-gold-leaf", weight: 25, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/docent-label", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },

  // ── C-rank family tables: altitude-haunts ──────────────────────────
  {
    id: "drop-table/altitude-haunts-regular",
    entries: [
      { itemId: "loot/monster-part/observation-lens", weight: 30, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/altitude-resin", weight: 20, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 10, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/ichor", weight: 10, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/altitude-haunts-elite",
    entries: [
      { itemId: "loot/monster-part/altitude-resin", weight: 25, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/observation-lens", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/crystal-eye", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 2 },
    ],
  },

  // ── C-rank boss tables ─────────────────────────────────────────────
  {
    id: "drop-table/the-market-maker",
    entries: [
      { itemId: "loot/monster-part/exchange-coin", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/ticker-ribbon", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/market-scythe", weight: 6, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/exchange-bespoke", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/margin-cufflinks", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 9, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/the-concierge-prime",
    entries: [
      { itemId: "loot/monster-part/penthouse-velvet", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/residence-key", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/concierge-baton", weight: 6, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/residence-longcoat", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/residence-keyring", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 9, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/the-foreman-prime",
    entries: [
      {
        itemId: "loot/monster-part/corporate-lanyard",
        weight: 20,
        minQuantity: 2,
        maxQuantity: 4,
      },
      { itemId: "loot/monster-part/corridor-glass", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/skybridge-rail", weight: 6, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/skybridge-carrier", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/corridor-goggles", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 9, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/the-stationmaster",
    entries: [
      { itemId: "loot/monster-part/platform-rail", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/express-token", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/express-pike", weight: 6, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/skybridge-carrier", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/corridor-goggles", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 9, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/the-curator-prime",
    entries: [
      { itemId: "loot/monster-part/gallery-gold-leaf", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/docent-label", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/curator-stylus", weight: 6, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/gallery-smock", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/margin-cufflinks", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 9, minQuantity: 1, maxQuantity: 3 },
    ],
  },
  {
    id: "drop-table/the-altitude-keeper",
    entries: [
      { itemId: "loot/monster-part/altitude-resin", weight: 20, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/observation-lens", weight: 15, minQuantity: 2, maxQuantity: 4 },
      { itemId: "loot/monster-part/spire-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "loot/monster-part/void-residue", weight: 15, minQuantity: 1, maxQuantity: 2 },
      { itemId: "weapon/altitude-spike", weight: 6, minQuantity: 1, maxQuantity: 1 },
      { itemId: "outfit-overlay/gallery-smock", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "accessory/altitude-earpiece", weight: 5, minQuantity: 1, maxQuantity: 1 },
      { itemId: "loot/monster-part/threat-gland", weight: 10, minQuantity: 1, maxQuantity: 3 },
      { itemId: "loot/monster-part/crystal-eye", weight: 9, minQuantity: 1, maxQuantity: 3 },
    ],
  },
] satisfies readonly DropTable[];
