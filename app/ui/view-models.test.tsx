...
describe("public pressure and rival views", () => {
  it("maps public pressure into score bands, district labels, and relationship cooldowns", () => {
    const pressure = buildPublicPressureView(
      {
        score: 76,
        dominantSource: "press",
      ...
      [{ factionId: "faction/city-licensing", standing: -12, cooldownUntilTick: 300 }],
    );

    expect(pressure.band).toBe("crackdown");
    expect(pressure.dominantSource).toBe("press");
    expect(pressure.districts[0]).toEqual(
      expect.objectContaining({
    ...
    expect(pressure.factionRelationships[0]).toEqual(
      expect.objectContaining({
  ...
  it("uses runtime-ready rival records for the current rival readout", () => {
    const primaryRival = readyToWireRivals[0];
    expect(primaryRival).toBeDefined();

    const rival = buildCurrentRivalView({
      active: true,
    ...
    expect(rival).toEqual(
      expect.objectContaining({
        rivalId: primaryRival!.id,
        shortDisplayName: primaryRival!.shortDisplayName,
        leaderName: primaryRival!.leader.name,
        leaderPortrait: primaryRival!.assetPaths!.leaderPortrait,
        intensity: 64,
      }),
    );
  });
