  ...
  switch (surface) {
    case "incident-framing":
      return [
        ...
        "- The player is the manager. A named in-world presenter delivers every incident brief to the player. The payload names that presenter, their role, a short voice brief, and their current emotional expression. Write the briefing and every choice field in that presenter's voice at that emotional register.",
        "",
        ...SHARED_PROMPT_SECTIONS,
        ...INCIDENT_PROMPT_SECTIONS,
        "PRESENTER VOICE",
        "- The payload's `presenter` block is authoritative. The simulation already chose the presenter; do not substitute, rename, or ignore them.",
        "- Anchor the briefing register on the presenter's role and voice brief. If the cook is briefing the player, the copy sounds like the cook would sound. If the compliance officer is briefing, the copy sounds like a compliance officer.",
        "- The `presenter.expression` field modulates register. `neutral` is matter-of-fact institutional. `concerned` is tighter, worried, blunt. `serious` is clipped, escalated, no jokes. `amused` allows dry, institutional humor where the incident category permits it.",
        "- Never break frame to quote, describe, or reference the presenter by name inside the briefing or choice text. The copy is what they say, not narration about them.",
        "- Keep the presenter's voice consistent across briefing, descriptions, consequenceSummary, and resolutionSummary.",
        "",
        "TASK",
        ...
        "",
        "STEP ORDER",
        "1. Absorb the deterministic facts: presenter identity and expression, incident template, operators, relationship, room, building, choice ids, and effects.",
        "2. Pick one grounded workplace angle (staffing tension, compliance friction, room culture, scheduling pressure, or relationship static) that is plausible for the named presenter to surface.",
        "3. Anchor the framing in small-business operations, in the presenter's voice at the given expression. Reference concrete physical detail when the room or building is named.",
        "4. Rewrite each provided choice so it reads like a management action in the same presenter voice, while preserving its deterministic meaning. Produce exactly one choice object per provided choiceId.",
        "5. Check the comedy-vs-tragedy mode against the incident category and tags, and adjust the register accordingly. `serious` expression never cracks jokes.",
        ...
        "- Briefing is 2 to 4 sentences, present tense unless recapping, and reads like the named presenter at the given expression.",
        '- The presenter is never named or described in the prose (no "Mara says", no "the cook mentions").',
...
function buildIncidentPromptPayload(payload: IncidentFramingPayload): Record<string, unknown> {
  return {
    ...
    presenter: {
      ...
      name: payload.presenter.name,
      roleTitle: payload.presenter.roleTitle,
      voiceBrief: payload.presenter.voiceBrief,
      domainSummary: payload.presenter.domainSummary,
      expression: payload.presenter.expression,
    },
    incident: {
  ...
  switch (surface) {
    case "incident-framing": {
      ...
      return [
        ...
        "- The `presenter` block names the in-world voice delivering this brief to the player. Write every copy field as that presenter at their current `expression`. Do not name or describe the presenter inside the prose.",
        "",
        ...
        "- Honor the presenter's voiceBrief and domainSummary: the register, vocabulary, and worry level should sound like that specific staff member at their current emotional expression.",
        "- If the incident category or tags signal death, catastrophic injury, or permanent loss, drop the comedy entirely regardless of expression.",
        "",
